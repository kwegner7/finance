...
class HtmlModern(UsualDefaults):
    ...
    def __init__(self, db, folder_html=None):
        self.db = db
        self.section_field = db.sectionChange()[-1]
        self.subsection_field = db.subsectionChange()[-1]
        self.section_title = self.section_field
        self.subsection_title = self.subsection_field
...
class HtmlModernSummary(HtmlModern):
    ...
    def columns(self):
        ...
        if True:
            nature_of_columns.append(self.cfg('Category'     , self.section_title    , 0, 'nowrapl', '1.0' ))
        if True:
            nature_of_columns.append(self.cfg('Subcategory'  , self.subsection_title , 0, 'nowrapl', '1.0' ))
